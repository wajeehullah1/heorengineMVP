 ...
 * AutoFillModal.jsx
 *
 * Full-screen overlay modal for the "Quick Start — Auto-fill from Evidence" flow.
 *
 * Props:
 *   onComplete(result)  — called with an AutoFillResult when auto-fill succeeds
 *   onSkip()            — called when user dismisses the modal without auto-filling
 */

import { useState, useRef } from "react";
import { handleAutoFill, PROGRESS_LABELS } from "../utils/autoFill.js";

// ---------------------------------------------------------------------------
// Shared style tokens (matches the existing HEOR Engine dark theme)
// ---------------------------------------------------------------------------
const NAVY      = "#0d2044";
const GREEN     = "#52b788";
...
export default function AutoFillModal({ onComplete, onSkip, initialInputs = {}, onFieldsChange }) {
  const [fields, setFields] = useState({
    deviceName: initialInputs.deviceName ?? "",
    indication: initialInputs.indication ?? "",
    cost:       initialInputs.cost       ?? "",
    benefits:   initialInputs.benefits   ?? "",
  });
  ...
  const handleSubmit = async () => {
    setStatus("loading");
    setError(null);
    setProgress(PROGRESS_LABELS.queued);
      ...
      <div style={card}>
        ...
        <div style={header}>
          ...
          <div>
            ...
            <div style={logoSub}>Auto-fill from Evidence</div>
          </div>
        ...
        <div style={heroSection}>
          ...
          <div>
            <h2 style={heroTitle}>Quick Start with AI Evidence</h2>
            <p style={heroDesc}>
              Describe your device and we'll search PubMed, NICE guidance, and
              NHS Reference Costs to pre-fill realistic estimates.
        ...
        {/* ── Footer note ── */}
        <p style={footerNote}>
          Searches PubMed · NICE Technology Appraisals · NHS Reference Costs
          · You can review and edit every value before submitting
        </p>
      </div>
